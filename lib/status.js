    ...
    child = exec("ps -ef",
        function (error, stdout, stderr) {
            ...
            if (out && out != '') {
                ...
                //查询所有服务的状态
                if (out.indexOf('nginx') > 0) {
                    service_s.filemanager = true;
                }

                if (out.indexOf('ffmpeg') > 0) {
                    service_s.imageview = true;
                }
                ...
                if (out.indexOf('srs') > 0) {
                    service_s.vlserver = true;
                }

                if (out.indexOf('mocar') > 0) {
 ...
 * @param params
 */
function settime(req, res, params) {

    var result = new Object();

    child = exec("date -s '" + params.time + "'",
        function (error, stdout, stderr) {
            var out = stdout;
            if (out && out != '') {
                var arr = out.replace(',', "").replace("\n", "");
